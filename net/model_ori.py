...
class Model():
    ...
    def __init__(self, sess, lambda_s, lambda_f, lambda_tv, learning_rate):
        ...
        self.vgg_o = vgg.Vgg19()
        self.vgg_s = vgg.Vgg19()
        self.vgg_c = vgg.Vgg19()
        self.__build_net__()
    ...
    def __build_net__(self):
        ...
        self.Output = Network(self.Content)
        
        o_features = self.__Output_Loss__()
        
        loss_s = self.__Style_Loss__(o_features)
        loss_c = self.__Content_Loss__(o_features)
        loss_tv = self.lambda_tv * total_variation_regularization(self.Output)
        
        

        
        self.Loss = loss_s + loss_c + loss_tv
        self.Optim = tf.train.AdamOptimizer(self.learning_rate).minimize(self.Loss)
        
    def __Output_Loss__(self):
        with tf.variable_scope('out_vgg'):
            self.vgg_o.build(self.Output/255.0)
        return [self.vgg_o.conv1_2, self.vgg_o.conv2_2, self.vgg_o.conv3_3, self.vgg_o.conv4_3, self.vgg_o.conv5_3]
    
    def __Style_Loss__(self, o_features):
        with tf.variable_scope('style_vgg'):
            self.vgg_s.build(self.Style/255.0)
        s_features = [self.vgg_s.conv1_2, self.vgg_s.conv2_2, self.vgg_s.conv3_3, self.vgg_s.conv4_3, self.vgg_s.conv5_3]
        ...
        for g, g_ in zip(gram, gram_):
            loss_s +=  self.lambda_s * tf.reduce_mean(tf.square(tf.subtract(g, g_)), [1, 2])
    ...
    def __Content_Loss__(self, o_features):
        ...
        c_features = [self.vgg_c.conv1_2, self.vgg_c.conv2_2, self.vgg_c.conv3_3, self.vgg_c.conv4_3, self.vgg_c.conv5_3]
        
        loss_f = tf.zeros(1, tf.float32)
        for f, f_ in zip(c_features, o_features):
            loss_f +=  self.lambda_f * tf.reduce_mean(tf.square(tf.subtract(f, f_)), [1, 2, 3])
            
        return loss_f
    
    def train(self, style, content):
        return self.sess.run([self.Loss, self.Optim], feed_dict = {self.Style: style,
